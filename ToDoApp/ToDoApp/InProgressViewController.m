...
- (void)viewDidLoad {
    ...
    _isSorted = NO;
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadTasks) name:@"TaskUpdatedInProg" object:nil];
...
- (void)reloadTasks {
    ...
    NSError *error = nil;
        ...
        NSData *savedLow = [_defaults objectForKey:@"lowInProg"];
        NSData *savedMed = [_defaults objectForKey:@"medInProg"];
        NSData *savedHigh = [_defaults objectForKey:@"highInProg"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CustomTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cellInProg" forIndexPath:indexPath];
    Task *task;
    if(_isSorted){
        switch (indexPath.section) {
            case 0:
                task = _low[indexPath.row];
                cell.inProgView.backgroundColor = [ UIColor systemGreenColor];
                cell.inProgLabel.text = task.name;
                break;
            case 1:
                task = _medium[indexPath.row];
                cell.inProgView.backgroundColor = [ UIColor systemYellowColor];
                cell.inProgLabel.text = task.name;
                break;
            default:
                task = _high[indexPath.row];
                cell.inProgView.backgroundColor = [ UIColor systemRedColor];
                cell.inProgLabel.text = task.name;
                break;
        }
